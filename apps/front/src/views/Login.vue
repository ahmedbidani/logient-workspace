<template>
      ...
      <div class="w-full max-w-xs">
        <form @submit.prevent="onSubmit" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          <div class="flex items-center justify-between mt-4">
            <p class="inline-block align-baseline text-sm text-blue-500">Dont't have an account?
        ...
        <p class="text-center text-gray-500 text-xs">
          &copy;2023 Movies App by Logient. All rights reserved.
        </p>
      </div>
    </div>
  </template>
  ...
  const onSubmit = async () => {
    try {
      loading.value = true;
      const response = await axios.post(`http://localhost:3333/api/v1/security/signin`, form)
